...
class QueryBuilder
{
    ...
    /**public function having($condition, $value)   //esta funcion no funciona
    {
    ...
    //funcion para hacer having modificada ------------
...
// Ejemplo de uso:
require_once "pdo/config_pdo.php";
...
//modificacion de la consulta compleja
$resultados = $qb->table('productos p')
    ->select('c.id', 'GROUP_CONCAT(p.id) as productos_ids', 'c.nombre as categoria', 'MAX(p.precio) as precio')
    ->join('categorias c', 'p.categoria_id', '=', 'c.id')
    ->where('p.precio', '>', 100)
    ->whereIn('c.id', [1, 2, 3])
    ->groupBy('c.id') 
    ->having('COUNT(*)', '>=', 2)
    ->orderBy('MAX(p.precio)', 'DESC')
    ->limit(10)
    ->execute();

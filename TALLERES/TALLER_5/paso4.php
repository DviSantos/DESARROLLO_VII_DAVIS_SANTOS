...
function buscarLibros($libros, $termino) {
    ...
    return array_filter($libros, function($libro) use ($termino) {
        return strcasecmp($libro['titulo'], $termino) === 0 || strcasecmp($libro['autor'], $termino) === 0;
    });
}
...
function generarReporteBiblioteca($biblioteca) {
    // Tu código aquí
        ...
        return [
            ...
            'libros_ciencia_ficcion' => count($librosCienciaFiccion),
            'autor_mas_libros' => $autores[count($autores) - 1]
        ];
}
